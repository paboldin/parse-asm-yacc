...
typedef void (*rb_free_node_fn_t)(struct rb_node *);

struct rb_root {
	struct rb_node *rb_node;
	rb_cmp_fn_t cmp_cb;
	rb_free_node_fn_t free_node_cb;
};


#define rb_parent(r)   ((struct rb_node *)((r)->__rb_parent_color & ~3))

#define RB_ROOT(cmpfunc, freefunc)	(struct rb_root) { NULL, cmpfunc, freefunc }
			   ...
			   rb_free_node_fn_t free_node_cb)
{
	root->rb_node = NULL;
	root->cmp_cb = cmp_cb;
	root->free_node_cb = free_node_cb;
}
			       ...
			       unsigned long key)
{
	...
	rb_cmp_fn_t cmp_fn = root->cmp_cb;
...
void rb_destroy(struct rb_root *root)
{
	...
	rb_free_node_fn_t free_node_cb = root->free_node_cb;
